...
from env_history import Agent, click_and_type, turn_to_next_page, MAX_ACTION_STEP
...
def run_trail(args, env, rule_manager, skill_bank, Worker_agent):
    ...
    if is_success:
        if direct_success and cur_step>=1:
            respond = Worker_agent.generate(observation+worker_organize_prompt)
            print(respond)
            respond_thought, respond_code = process_respond(respond)
        ...
        if error_step >= 1:
            failure_case = "code error"
        elif env.action_step >= MAX_ACTION_STEP or cur_step>=MAX_REPLAN_STEP-1:
            failure_case = "inefficient"
        else:
        ...
        skill_bank.add_failure(env.task_type, env.task, env.init_obs, respond, failure_case)
    skill_bank.save()
